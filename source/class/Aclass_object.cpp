...
//----テクスチャを与える--------
void class_ObjectA::LoadTexture(const char *texture)
{
	// テクスチャの読み込み
...
//----描画処理--------
void class_ObjectG::Draw(LPDIRECT3DTEXTURE9 texture)
{
	LPDIRECT3DDEVICE9 pDevice = GetDevice();
	D3DXMATRIX mtxView, mtxScale, mtxTranslate, mtxWorld;
...
//----頂点作成--------
int class_ObjectG::MakeVertex(void)
{
	// オブジェクトの頂点バッファを生成
	if (FAILED(GetDevice()->CreateVertexBuffer(sizeof(VERTEX_3D) * NUM_VERTEX,	// 頂点データ用に確保するバッファサイズ(バイト単位)
		D3DUSAGE_WRITEONLY,		// 頂点バッファの使用法　
...
//----頂点座標設定--------
//@頂点位置のみ
void class_ObjectG::SetVertex(void)
{
	{//頂点バッファの中身を埋める
		VERTEX_3D *pVtx;
...
//@頂点色込み
void class_ObjectG::SetVertex(D3DXCOLOR color)
{
	{//頂点バッファの中身を埋める
		VERTEX_3D *pVtx;
...
//----バッファ系開放--------
void class_ObjectG::ReleaseBuffer(void)
{
	if (VtxBuff != NULL)
	{	// 頂点の開放
		VtxBuff->Release();
